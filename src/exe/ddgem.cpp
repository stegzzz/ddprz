...
int main(int argc, char *argv[]) {
  ...
  /*  std::cout << "hello sworld" << std::endl;
   try {
  ...
  // 		con = driver->connect("localhost", "stegzzz", "zo7WOnFkS2");
  /* Connect to the MySQL test database */
  // con->setSchema("stegzzz");
  // const char *dc = "date";
  // const char *cc = "comment";
  // stmt = con->createStatement();
  // res = stmt->executeQuery(
  //     "SELECT date,comment from exerciselog where exercise = 'weight'");
  // while (res->next()) // {
  //   records.push_back(res->getString("date") + delim +
  //                     res->getString("comment"));
  // std::cout << "\t... MySQL replies: ";
  /* Access column data by alias or column name */
  // std::cout << res->getString("_message") << endl;
...
void runCGI() {
  ...
  //          << std::endl;
  //   // cgicc::html()
  //   //        << cgicc::head(cgicc::title("cgicc example")) << std::endl;
  //   // output << cgicc::body() << "ello not open" << std::endl;
  //   // output << cgicc::body() << cgicc::html() << std::endl;
  ...
  //                                     100}; //!< probilistic response
  //                                     generation
  // std::seed_seq seeds{rd(), rd(), rd(), rd(), rd()};
  // rnd.seed(seeds);
  // // auto it = s2.begin();
  // // auto ite = s2.end();
  // // std::for_each(it, ite, [&ud, &rnd](double &r) { r = ud(rnd); });
  // for (auto &d : s2)
  //   d = ud(rnd);
  cgicc::Cgicc formData;
  // formData.save("tmp.env");
  std::cout << preHTMLString;
  // std::cout << cgicc::HTTPHTMLHeader() << std::endl;
  // std::cout << cgicc::html() << cgicc::head(cgicc::title("cgicc example"))
  //           << std::endl;
  // std::cout << cgicc::body() << "ello" << std::endl;
  // std::cout << cgicc::body() << cgicc::html() << std::flush;

  // } catch (const std::exception &e) {
  //   std::ofstream
  //   ouf{"c:/users/admin/workspace/cpp2020/dd/build/errorMsg.txt"}; ouf <<
  //   spgxyz::preHTMLString << e.what() << spgxyz::postHTMLString;
  //   ouf.close();
  // }
  // return;
  ...
  // #ifdef DEBUG
  //   // debug only, try to capture any xml hedwig sends
  //   xmlParseError(post);
  //   return;
  // #endif // DEBUG
  ...
  // now we have the xml posted data
  // if DEBUG we'll print it out
  // #ifdef DEBUG
  //   tinyxml2::XMLPrinter printer{};
  //   doc.Print(&printer);
  //   std::cout << preHTMLString;
  //   std::cout << "<textarea rows=\"75\" cols=\"150\" style
  //   =\"border:none;\">\n"; std::cout << "DEBUG on"
  //             << "\n";
  // #ifdef AWS
  //   std::cout << "From AWS"
...
/* mysqlData::mysqlData (mysqlData&& lhs){
  id=std::move(lhs.id);
  kss=std::move(lhs.kss);
  ksl=std::move(lhs.ksl);
  kssml=std::move(lhs.kssml);
  kslml=std::move(lhs.kslml);
  gs=std::move(lhs.gs);
  gl=std::move(lhs.gl);
  XMLPtr=std::move(lhs.XMLPtr);
  XMLSz
} */
